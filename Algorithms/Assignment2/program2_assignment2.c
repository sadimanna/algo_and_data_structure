...
int *sprague_grundy_multi_heap_optimal_move(int *N, int *M, int k)
{
	...
	{
		rem = N[i]%(M[i]+1);
		binlen = get_maxlen(rem);
		if (binlen > kth_bit)

			binstr = dec2bin(rem,binlen);
		else
		{
			binstr = dec2bin(rem,kth_bit);
			binlen = kth_bit;
		}

		if(binstr[binlen - kth_bit]==1)
		{
			move[0] = i;

			if (binlen > kth_bit)
				move[1] = rem-bin2dec(xor(dec2bin(nim_sum_sg,binlen),dec2bin(rem,binlen),binlen),binlen);
			else
				move[1] = rem-bin2dec(xor(dec2bin(nim_sum_sg,kth_bit),dec2bin(rem,kth_bit),kth_bit),kth_bit);
